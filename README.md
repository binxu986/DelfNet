...
…
```
Cluter表示一簇神经元，可以看作是一个独立的神经网络功能区域（或者看成是一层神经网络，有独立更新的一批神经元），簇内的神经元之间进行信息交换。继续沿用Block的概念，是为了和Transformer Block概念相对应，方便理解。如Cluster0中的Block0, Block1, … 表达了这一簇神经元随时间t的更新变化情况。当一个Block就是一个Cluster时，就退化到原始的transformer结构了。通过建模神经元簇在时间t上的变化，我们就可以有机会去对神经元状态进行“时间编码”，这一部分是当前神经网络结构没有显示表达的（当前仅打开了可能，但具体如何做，还是开放性问题）。

![Fig1](./figs/neuron_cluster_network.png "神经元簇内部网络示意图")
...
核心优势：
- 表达上是可以完全兼容原来的网络权重参数的，且总的参数量规模没有变化；
- 更新后序 Block 的变量时，dy 会直接作用于前序 Block 的变量，产生**联动效果**
- 如果$△$很稀疏，或者有规律变化，就可能可以实现对参数的有效压缩，以大幅缓解decoding带宽bound问题，以及模型权重的存储问题。



### 🕐 时间编码

因为有了时间步$\Delta_i$的变化，我们就可以对神经元簇在时间t上的变化进行记录（hidden states over time），把这些hidden states变化序列记录下来，我们就可以对神经元簇在时间t上的变化进行编码。（目前还只是思路，具体实现还需要研究，比如可以在hidden states上进行简单的卷积操作，输出和原来一样大小的time-encoded hidden states,作为下一个block的输入，这样可以保持原始的计算结构不变，等等。）
